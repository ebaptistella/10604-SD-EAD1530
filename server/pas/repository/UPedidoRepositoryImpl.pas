...
const
  CMD_INSERT_PEDIDO
    : String =
    'INSERT INTO tb_pedido (cd_cliente, dt_pedido, dt_entrega, vl_pedido, nr_tempopedido, cd_tamanho, cd_sabor) VALUES (:pCodigoCliente, :pDataPedido, :pDataEntrega, :pValorPedido, :pTempoPedido, :pTamanho, :pSabor)';
...
begin
  FFDQuery.Close;
  FFDQuery.SQL.Clear;
  FFDQuery.SQL.Add('select nr_tempopedido, vl_pedido, cd_tamanho, cd_sabor from tb_pedido where cd_cliente = :pCodigoCliente' );
  FFDQuery.ParamByName('pCodigoCliente').AsInteger := strtoint(ADocumentoCliente);
  FFDQuery.Open;
  if FFDQuery.IsEmpty then
     result := nil
  else
    Result := TPedidoRetornoDTO.Create(TPizzaTamanhoEnum(FFDQuery.FieldByName('cd_tamanho').AsInteger), TPizzaSaborEnum(FFDQuery.FieldByName('cd_sabor').AsInteger), FFDQuery.FieldByName('vl_pedido').AsFloat, FFDQuery.FieldByName('nr_tempopedido').AsInteger);
